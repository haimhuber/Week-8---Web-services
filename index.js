function createDatalist() {
    fetch('https://data.gov.il/api/action/datastore_search?resource_id=b7cf8f14-64a2-4b33-8d4b-edb286fdbd37&limit=1500')
        ...
        .then((allIsraelCitiesAsObj) => {
            ...
            const citiesDataList = document.createElement('datalist');
            citiesDataList.setAttribute('id', 'cities');
            for (let index = 0; index < israelCities.length; index++) {
                const newCityAddToList = document.createElement('option');
                newCityAddToList.setAttribute('value', israelCities[index]["שם_ישוב_לועזי"].trim());
                citiesDataList.appendChild(newCityAddToList);
            }
            console.log(citiesDataList);
            document.querySelector("#cityDataList").appendChild(citiesDataList);
        })

}

function sleep(ms) { // Set delay
    return new Promise(resolve => setTimeout(resolve, ms));
}

async function weatherInfo() {
    document.querySelector("#buildDivInDOM").textContent = "";
    const loadingText = document.createElement('h2');
    loadingText.textContent = "Lodaing";
    loadingText.classList.add('flash');
    document.querySelector("#buildDivInDOM").appendChild(loadingText);
    await sleep(2000);
    ...
    if (city === "") {
        ...
        fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${apiKey}&units=metric`)
            ...
            .then((weatherDataAsObj) => {
                ...
                document.querySelector("#buildDivInDOM").append(weatherDiv);
            });
    }
